const translations = {
  en: {
    ...
    configHealthReasonRouteFinal: "fallback route points to an invalid outbound",
  ...
  zh: {
    ...
    configHealthReasonRouteFinal: "兜底出口指向无效出站",
...
function renderMaintenance() {
  ...
  rows.appendChild(renderMaintenanceDetails(t("configHealthTitle"), [
    ...
    [t("routeFinal"), configHealth.routeFinal || t("unknown"), configHealth.routeFinalValid === false ? "warn" : "good"],
...
function buildNodeFromForm() {
  ...
  if (type === "hysteria2") {
    ...
    if (transportMode === "brutal") {
      outbound.multiplex = outbound.multiplex || { enabled: true, protocol: "h2mux", padding: true, max_connections: 4 };
